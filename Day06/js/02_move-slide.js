...
$(function() {
    ...
    let currentNo = 1               //활성화된 인덱스 번호
    let currentIndex = 0            //활성화된 인덱스
    ...
    // 투명도 세팅
    $('.slide-item').css({opacity: 0.2})
    $('.slide-item:nth-child(' + (initNum+1) + ')').css({opacity: 1 })


    //이전 버튼 클릭 이벤트
    $('.prev').on('click',()=>{
        //prependTo(부모요소) : 부모요소의 가장 앞의 자식요소로 추가하는 메서드
        $('.slide-item:last-child').prependTo(list)
    ...
    $(document).on('click','.dot', function() {
        ...
        //활성화된 위치보다 앞의 이미지를 선택한 경우 (next방향)
        if(gap > 0){
            list.animate({left: currentPosition -= slideWidth*absGap},500, function(){
                //애니메이션 종료 후 작업
                for( let i = 0 ; i < absGap ; i++ ){
